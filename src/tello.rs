...
impl Tello {
    pub fn new() -> Result<Self, std::io::Error> {
        let command_socket = UdpSocket::bind("127.0.0.1:9001")?;
        let state_socket = UdpSocket::bind("127.0.0.1:8890")?;

        command_socket.set_read_timeout(Some(Duration::from_millis(100)));
        state_socket.set_read_timeout(Some(Duration::from_millis(100)));
    ...
    pub fn connect(&mut self) {
        ...
        self.command_thread = Some(std::thread::spawn(move || {
            while command_running.load(Ordering::SeqCst) {
                ...
                let mut socket = command_socket.lock().unwrap();

                match socket.recv(&mut buffer) {
                    Ok(size) => {
                        let string = std::str::from_utf8(&buffer[..size]).unwrap_or_default().trim();
                    }
                    Err(error)=> {
        ...
        self.state_thread = Some(std::thread::spawn(move || {
            while state_running.load(Ordering::SeqCst) {
                let mut buffer: [u8; 1500] = [0; 1500];
                let mut socket = state_socket.lock().unwrap();

                match socket.recv(&mut buffer) {
                    Ok(size) => {
                        let string = std::str::from_utf8(&buffer[..size-1]).unwrap_or_default().trim();
    ...
    pub fn up(&self, distance_cm: u16) {
        self.send_command(format!("up {}", distance_cm).as_str());
    }

    pub fn down(&self, distance_cm: u16) {
        self.send_command(format!("down {}", distance_cm).as_str());
    }

    pub fn left(&self, distance_cm: u16) {
        self.send_command(format!("left {}", distance_cm).as_str());
    }

    pub fn right(&self, distance_cm: u16) {
        self.send_command(format!("right {}", distance_cm).as_str());
    }

    pub fn forward(&self, distance_cm: u16) {
        self.send_command(format!("forward {}", distance_cm).as_str());
    }

    pub fn back(&self, distance_cm: u16) {
        self.send_command(format!("back {}", distance_cm).as_str());
    }

    pub fn cw(&self, angle_millidegs: u16) {
        self.send_command(format!("cw {}", angle_millidegs).as_str());
    }

    pub fn ccw(&self, angle_millidegs: u16) {
        self.send_command(format!("ccw {}", angle_millidegs).as_str());
    }

    pub fn flip(&self, flip: Flip) {
        self.send_command(format!("flip {}", flip.value()).as_str());
    }

    pub fn go(&self, x_cm: u16, y_cm: u16, z_cm: u16, speed: u8) {
        self.send_command(format!("go {} {} {} {}", x_cm, y_cm, z_cm, speed).as_str());
    }

    pub fn curve(&self, x1_cm: u16, y1_cm: u16, z1_cm: u16, x2_cm: u16, y2_cm: u16, z2_cm: u16, speed: u8) {
        self.send_command(format!("curve {} {} {} {} {} {} {}", x1_cm, y1_cm, z1_cm, x2_cm, y2_cm, z2_cm, speed).as_str());
    }

    pub fn rc(&self, left_right: i8, forward_backward: i8, up_down: i8, yaw: i8) {
        self.send_command(format!("rc {} {} {} {}", left_right, forward_backward, up_down, yaw).as_str());
    }

    pub fn speed(&self, speed_cms: u8) {
        self.send_command(format!("speed {}", speed_cms).as_str());
    }

    pub fn wifi(&self, ssid: &str, password: &str) {
